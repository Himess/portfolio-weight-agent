...
Codex mcp add --transport http portfolio-weight-agent https://portfolio-weight-agent.vercel.app/api/mcp

# Or from a clone, over stdio
Codex mcp add portfolio-weight-agent -- node --env-file-if-exists=.env --import tsx src/mcp/stdio.ts
```

Pair it with Binance's own server, which is where balances and orders live:

```bash
Codex mcp add --transport http binance https://agent.binance.com/mcp/agentic
```
